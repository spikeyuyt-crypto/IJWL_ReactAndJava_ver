...
public class A002WordsService {
        ...
        public List<A002GetWordsListBean> getAllWords(
                ...
                List<A002GetWordsListBean> words = a002WordsMapper.getAllWords(a002GetWordsRequestDto);
        ...
        public void markWord(
                        ...
                        int userId) {
                if (a002MarkWordDto.isEmpty()) {
                        throw new BusinessException(
                                        400,
                                        "マーク単語を指定してください");
                }

                List<Integer> existingWordIds = a002WordsMapper.getMarkedWords(userId)
                                .stream()
                                .map(A002GetWordsListBean::getWordId)
                                .toList();

                List<A002MarkWordDto> insertTargets = a002MarkWordDto.stream()
        ...
        public void unmarkWord(List<A002UnmarkAndDeleteWordDto> a002UnmarkAndDeleteWordDto, int userId) {
                if (a002UnmarkAndDeleteWordDto.isEmpty()) {
                        throw new BusinessException(
                                        400,
                                        "マーク対象選択していません");
                }
        ...
        public void recordBatsuWord(List<A002RecordBatsuWordDto> a002RecordBatsuWordDto, int userId) {
                ...
                List<Integer> dbBatsuWords = a002WordsMapper.getBatsuWords(userId)
                                .stream()
                                .map(A002GetWordsListBean::getWordId)
                                .toList();

                List<A002RecordBatsuWordDto> insertTargets = a002RecordBatsuWordDto.stream()
                                .collect(Collectors.toMap(
                ...
                int insertedRows = a002WordsMapper.recordBatsuWord(insertTargets, userId);
